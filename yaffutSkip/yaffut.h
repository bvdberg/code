...
{
  bool enabled;
  ITest(bool enabled_ = true) : enabled(enabled_) {}
  virtual ~ITest(){}
};
...
private:
  ...
  typedef std::map<std::string, bool> TestsEnabled_t;
  Tests_t m_Tests;
  TestsEnabled_t m_TestsEnabled;
  size_t m_fail;
  size_t m_pass;
  size_t m_skip;
...
public:
  ...
  void RegisterSkip(const std::string& name, bool enabled)
  {
    m_TestsEnabled[name] = enabled;
  }
        ...
        try
        {
          std::cout << std::endl << it->first << ' ' << std::flush;
            if (!it->second()->enabled) {
                std::cout << COLOR_SKIP"[SKIPPED]"COLOR_NORMAL << std::flush;
                ++m_skip;
            } else {
                {
  ...
  void Report ()
  {
    ...
    std::cout << std::endl;
    std::cout << "[TOTAL] (" << m_pass + m_fail + m_skip << '/' << size << ")   ";
    std::cout << COLOR_OK << "OK " << m_pass << COLOR_NORMAL;
    if (m_skip)
        std::cout << " | " << COLOR_SKIP"SKIPPED " << m_skip << COLOR_NORMAL;
    if (m_fail)
        std::cout << " | " << COLOR_FAIL"FAIL " << m_fail << COLOR_NORMAL;
    std::cout << std::endl;
...
struct RegistratorSkip
{
  RegistratorSkip()
  {
    Factory::Instance().RegisterSkip(TestName());
  }
...
#define TEST_SKIP(Suite, Case)\
  namespace {\
      struct Case: public yaffut::Test<Suite, Case>{ \
          Case();\
          static yaffut::RegistratorSkip<Suite, Case> s_RegistratorSkip; \
    }; } \
  template struct yaffut::Test<Suite, Case>; Case::Case() : yaffut::Test<Suite, Case>(false) 
