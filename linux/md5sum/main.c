...
int main(int argc, const char *argv[])
{
    ...
    char* cp = region;
    char* linestart = region;
    int count = 0;
    while (cp != region + statbuf.st_size) {
        if (*cp == '\n') {
            *cp = 0;
            printf("checking file %d\r", count);
            handleLine(linestart);
            linestart = cp+1;
            count++;
        }
        cp++;
